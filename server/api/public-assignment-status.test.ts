...
import { getPublicAssignmentStatus, normalizeTrackingCode } from "./public-assignment-status.js"
...
describe("public assignment status", () => {
  ...
  it("uses one parameterized, explicitly scoped query", async () => {
    ...
    await getPublicAssignmentStatus(code.toLowerCase())
  ...
  it("returns null without querying for malformed IDs and unknown records", async () => {
    await expect(getPublicAssignmentStatus("99")).resolves.toBeNull()
    expect(db.initFilesDb).not.toHaveBeenCalled()
    db.query.mockResolvedValue([])
    await expect(getPublicAssignmentStatus(code)).resolves.toBeNull()
  })
  ...
  it("maps terminal and legacy values while calculating progress", async () => {
    ...
    await expect(getPublicAssignmentStatus(code)).resolves.toMatchObject({ status: "completed", progress: 100, progressStage: "humaned" })
    await expect(getPublicAssignmentStatus(code)).resolves.toMatchObject({ status: "not-started", progress: 0, progressStage: "ai-draft" })
  })
  ...
  it("serializes date-only values and returns exactly seven safe keys", async () => {
    ...
    const result = await getPublicAssignmentStatus(code)
    expect(result).toEqual({ trackingCode: code, category: null, status: "ongoing", progressStage: "humaned", progress: 30, dueDate: "2026-08-20", updatedAt: "2026-08-01T10:00:00.000Z" })
    expect(Object.keys(result ?? {})).toEqual(["trackingCode", "category", "status", "progressStage", "progress", "dueDate", "updatedAt"])
