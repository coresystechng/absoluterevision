...
import { formatPublicDueDate, getPublicMilestones, getPublicProgress, getPublicStageDetails, getPublicStatusLabel, normalizeTrackingCode, publicProgressStages } from "./public-assignment-status"
...
describe("public tracking helpers", () => {
  ...
  it("exposes the six client-friendly stage labels", () => {
    expect(publicProgressStages.map((stage) => stage.label)).toEqual([
      "Initial draft", "Expert editing", "Language review", "Originality review", "Formatting", "Final quality check",
    ])
